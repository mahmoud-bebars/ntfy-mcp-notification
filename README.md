...
**⚠️ Important**: ntfy.sh topics are **public by default**. Anyone who knows your topic name can:
- Subscribe to your notifications
- Send notifications to your topic

**Best Practices:**
1. ✅ Use unique, hard-to-guess topic names (e.g., `claude-notif-$(uuidgen | head -c 8)`)
2. ✅ Avoid including personal information in topic names
3. ✅ Consider self-hosting ntfy.sh for sensitive data
4. ❌ Don't use simple names like `claude`, `notifications`, or `yourname`

### What Gets Sent to ntfy.sh

**Sent:**
- Notification title and message (from tool parameters)
- Priority level
- Tags
- Timestamp (added by ntfy.sh)

**NOT Sent:**
- Claude conversation history
- User credentials
- File contents or source code
- Any data not explicitly passed to notification tools

**Reminder**: Treat notification content as potentially public. Avoid including sensitive data.

### Future: Private Topics

A future version will support ntfy.sh authentication for private topics using access tokens or username/password.
...
```bash
git clone https://github.com/yourusername/ntfy-mcp-notification.git
...
This opens a web interface to test the MCP server tools.
